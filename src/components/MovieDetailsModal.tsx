      ...
      <DialogBody>
        <div className="flex flex-col md:flex-row gap-4">
          ...
          <div className="flex-1">
            ...
            <Text>
              <Strong>Duration:</Strong> {parseDuration(movie.duration || '')}
            </Text>
            <Text>
              <Strong>Directors:</Strong> {movie.directors?.join(', ')}
            </Text>
            <Text>
              <Strong>Main Actors:</Strong> {movie.mainActors?.join(', ')}
            </Text>
            <Text>
              <Strong>Release Date:</Strong>{' '}
              {formatDate(movie.datePublished || '')}
            </Text>
            <Text>
              <Strong>Rating Value:</Strong> {movie.ratingValue} /{' '}
              {movie.bestRating}
            </Text>
            <Text>
